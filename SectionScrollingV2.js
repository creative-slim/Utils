...
(() => {
  ...
  window.Webflow.push(() => {
    ...
    const scrollSpeed = document.querySelector("[data-scroll-speed]") ? document.querySelector("[data-scroll-speed]").getAttribute("data-scroll-speed") : "1";
    if (!sections.length)
      return;
    let currentIndex = 0;
    let animating = false;
    ...
    Observer.create({
      ...
      wheelSpeed: -1,
      onDown: () => !animating && currentIndex > 0 && gotoSection(currentIndex - 1),
      onUp: () => !animating && currentIndex < sections.length - 1 && gotoSection(currentIndex + 1),
      tolerance: 10,
      preventDefault: true
    });
    gotoSection(0);
