...
(() => {
  ...
  window.Webflow ||= [];
  window.Webflow.push(() => {
    gsapWithCSS.registerPlugin(Observer, ScrollToPlugin);
    const scrollContainer = document.querySelector('[data-scroll="section-scroll"]');
    ...
    function createObserver(preventDefault) {
      if (observerInstance)
      ...
      observerInstance = Observer.create({
        ...
        onChange: (self) => {
          const { event, deltaY } = self;
          console.log(event.type);
          if (!isScrollSectionVisible || animating)
            return;
          if (event.type === "wheel") {
            if (deltaY > 0 && indexInsideBounds(currentIndex + 1)) {
              navigateSection(1);
            } else if (deltaY < 0 && indexInsideBounds(currentIndex - 1)) {
              navigateSection(-1);
            }
    ...
    createObserver(true);
    function indexInsideBounds(index) {
      return index >= 0 && index < totalSections;
    }
    function navigateSection(delta) {
      animating = true;
      currentIndex = Math.min(Math.max(currentIndex + delta, 0), totalSections - 1);
      gsapWithCSS.to(window, {
        ...
        onComplete: () => {
          animating = false;
          createObserver(currentIndex !== totalSections - 1);
        }
      });
    }
    function isScrollContainerVisible() {
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              console.log("Scroll container is visible");
              isScrollSectionVisible = true;
              createObserver(true);
            } else {
              console.log("Scroll container is not visible");
        ...
        {
          threshold: [0]
          // Trigger when even one pixel is visible
        }
      );
